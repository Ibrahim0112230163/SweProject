...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!courseTitle || !file || !userId) {
      return NextResponse.json(
        { error: 'Missing courseTitle, file, or userId' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (!outlineText || outlineText.trim().length < 50) {
        return NextResponse.json(
          {
            error:
              'PDF has no readable text (likely scanned/image-only PDF)',
          },
          { status: 400 }
        );
      }
...
{
  "trendingTopics": [
    { "topic": "", "demandLevel": "high|medium|low", "reason": "" }
  ],
  "skillGaps": [
    {
      "gap": "",
...
`;
    ...
    try {
      ...
      analysisData = {
        ...
        marketRelevanceScore: 0,
        summary:
          'Analysis completed, but structured data could not be extracted.',
      };
    }
    ...
    try {
      const path = `outlines/${userId}/${Date.now()}_${file.name}`;
